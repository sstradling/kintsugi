...
6. **Difficulty / progression model.** Themed packs, linear unlock, daily
   piece, all unlocked? Affects meta UI.
8. **Account & cross-device save.** Local-only, iCloud/Google
   Play Games Saved Games, or a custom backend?
9. **AR mode?** Cut for v1?
10. **Photo / share mode?** Probably yes for organic marketing.
11. **Filler unlock cadence.** All 8 from puzzle 1, or unlock progression?
12. **Audio direction.** Static ambient bed vs procedural generative?
13. **Content target for launch.** 10 / 25 / 50 puzzles?
14. **Live ops?** Daily/weekly free puzzle, seasonal filler skins?

### 6.3 Starter content (for pipeline bring-up)

Two synthetic puzzles will exercise the Blender → manifest pipeline before
any artist-authored content exists:

- **`cube_8`** — a unit cube fractured into 8 equal sub-cubes (2×2×2). The
  cleanest possible adjacency graph (each shard has exactly 3 neighbors).
- **`sphere_8_slices`** — a unit sphere sliced into 8 parallel sheets of
  equal thickness along the Y axis. Tests the pipeline against curved
  surfaces and a strictly linear adjacency graph (each shard has 1–2
  neighbors).

These are deliberately trivial so failures point at the pipeline, not the
content.
